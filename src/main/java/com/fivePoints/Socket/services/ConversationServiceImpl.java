...
public class ConversationServiceImpl implements ConversationService {
	...
	public Conversation getOneConversation(Long user1, Long user2) throws NoResultException {
		...
		TypedQuery<Conversation> query = (TypedQuery<Conversation>) em.createQuery(
				"FROM Conversation u WHERE u.user1.id = :user1 and u.user2.id = :user2", Conversation.class);
		conv = query.setParameter("user1", user1).setParameter("user2", user2).getResultList();
		if (conv.isEmpty() != true) {
			return conv.get(0);
		} else {
			TypedQuery<Conversation> query2 = (TypedQuery<Conversation>) em.createQuery(
					"FROM Conversation u WHERE u.user1.id = :user2 and u.user2.id = :user1", Conversation.class);
			...
			if (conv.isEmpty() != true) {
				...
				TypedQuery<Conversation> query3 = (TypedQuery<Conversation>) em.createQuery(
						"FROM Conversation	 u WHERE u.user1.id = :user1 and u.user2.id = :user2", Conversation.class);
